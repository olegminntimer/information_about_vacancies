...
class HeadHunterAPI(Parser):
    ...
    def optimized_list(self) -> list:
        """Метод выборки полей вакансий: id, название, ссылка, зарплата, валюта, требования"""
        optim = []
        for row in self.vacancies:
            if row["salary"]:
                ...
                salary = None
            optim.append({"id": row["id"], "name":row["name"], "url": row["alternate_url"], "salary" : salary, "requirement": row["snippet"]["requirement"]})
        return optim
